...
#### deployments to test
#envs=(wsmt1s.elasticbeanstalk.com wsmt2s.elasticbeanstalk.com wsmt4s.elasticbeanstalk.com wsmt1m.elasticbeanstalk.com wsmt2m.elasticbeanstalk.com wsmt4m.elasticbeanstalk.com)
envs=(devserver)
envs_count=${#envs[@]}
...
function produce_load() {
	#### build command
	if [ -d "autobench/$3/$1_results.csv" ]; then
		...
		autobench_command="autobench --single_host --host1 $1 --port1 $port --uri1 ws-mt/public/application/index/$2 --low_rate $low_rate --rate_step $rate_step --high_rate $high_rate --num_call $num_call --const_test_time $test_time --timeout $timeout --output_fmt $output_format --file autobench/$3/$1_results.csv"
	fi
